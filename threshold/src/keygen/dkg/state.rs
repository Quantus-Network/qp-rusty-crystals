...
impl Round1Data {
    ...
    pub fn add_contribution(&mut self, party_id: ParticipantId, contribution: [u8; SESSION_ID_SIZE]) {
...
impl Round3Data {
    ...
    pub fn add_contributions(&mut self, party_id: ParticipantId, contributions: PartyContributions) {
    ...
    pub fn all_verified(&self) -> bool {
        !self.verification_results.is_empty()
            && self.verification_results.values().all(|&v| v)
    }
}
...
impl DkgStateData {
    ...
    pub fn process_round1(&mut self, msg: DkgRound1Broadcast) -> Result<(), String> {
        if !matches!(self.state, DkgState::Round1Generating | DkgState::Round1Waiting) {
            return Err(format!(
                "Cannot process Round 1 message in state {}",
                self.state.name()
            ));
        }
    ...
    pub fn process_round2(&mut self, msg: DkgRound2Broadcast) -> Result<(), String> {
        if !matches!(self.state, DkgState::Round2Generating | DkgState::Round2Waiting) {
            return Err(format!(
                "Cannot process Round 2 message in state {}",
                self.state.name()
            ));
        }
        ...
        if self.round2.commitment_hashes.contains_key(&msg.party_id) {
            return Err(format!(
                "Duplicate Round 2 message from party {}",
                msg.party_id
            ));
        }
    ...
    pub fn process_round3(&mut self, msg: DkgRound3Broadcast) -> Result<(), String> {
        if !matches!(self.state, DkgState::Round3Revealing | DkgState::Round3Waiting) {
            return Err(format!(
                "Cannot process Round 3 message in state {}",
                self.state.name()
            ));
        }
        ...
        if self.round3.contributions.contains_key(&msg.party_id) {
            return Err(format!(
                "Duplicate Round 3 message from party {}",
                msg.party_id
            ));
        }
    ...
    pub fn process_round4(&mut self, msg: DkgRound4Broadcast) -> Result<(), String> {
        if !matches!(self.state, DkgState::Round4Confirming | DkgState::Round4Waiting) {
            return Err(format!(
                "Cannot process Round 4 message in state {}",
                self.state.name()
            ));
        }
        ...
        if self.round4.confirmations.contains_key(&msg.party_id) {
            return Err(format!(
                "Duplicate Round 4 message from party {}",
                msg.party_id
            ));
        }
...
mod tests {
    ...
    fn test_round4_consensus() {
        ...
        data.add_confirmation(0, DkgRound4Broadcast {
            party_id: 0,
            success: true,
            public_key_hash: [42u8; 32],
        });
        data.add_confirmation(1, DkgRound4Broadcast {
            party_id: 1,
            success: true,
            public_key_hash: [42u8; 32],
        });
        ...
        data.add_confirmation(2, DkgRound4Broadcast {
            party_id: 2,
            success: true,
