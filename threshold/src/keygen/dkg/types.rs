...
impl<S: TranscriptSigner> MithrilDkgConfig<S> {
    ...
    pub fn my_leader_subsets(&self) -> Vec<SubsetMask> {
        self.all_subsets()
            .into_iter()
            .filter(|&mask| self.is_leader(mask))
            .collect()
    }
    ...
    pub fn my_subsets(&self) -> Vec<SubsetMask> {
        self.all_subsets()
            .into_iter()
            .filter(|&mask| self.is_in_subset(mask))
            .collect()
    }
...
pub fn h_commit_pk(subset_mask: SubsetMask, partial_pk: &PartialPublicKey) -> [u8; COMMITMENT_HASH_SIZE] {
    let mut state = fips202::KeccakState::default();
...
pub fn compute_partial_output_hash(partial_pks: &HashMap<SubsetMask, PartialPublicKey>) -> [u8; 32] {
    let mut state = fips202::KeccakState::default();
