...
///
/// 1. **Round 1 (Commitment)**: Each party generates random values and broadcasts
///    a commitment hash. This prevents parties from adaptively choosing their
///    randomness based on others' values.
///
/// 2. **Round 2 (Reveal)**: Each party reveals their actual commitment values
///    and receives others' values. The message to be signed is incorporated here.
///
/// 3. **Round 3 (Response)**: Each party computes their signature share based on
///    their secret key share and the aggregated commitments.
///
...
/// - Only broadcast messages (`Round1Broadcast`, `Round2Broadcast`, `Round3Broadcast`)
...
impl ThresholdSigner {
    ...
    pub fn new(
        ...
        // Validate that the private key matches the config
        ...
        if private_key.total_parties() != config.total_parties() {
            return Err(ThresholdError::InvalidConfiguration(format!(
                "Private key total parties ({}) does not match config total parties ({})",
    ...
    pub fn round2_reveal_with_data(
        ...
        let other_commitment_data: Vec<Vec<u8>> = other_broadcasts
            .iter()
            .map(|r2| r2.commitment_data.clone())
            .collect();
