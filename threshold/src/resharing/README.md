...
**Abort recovery (in-memory-only callers)**: if a caller moved their only in-memory copy into `ResharingConfig` with no external durable store, a failed or stalled session has produced no replacement share, and Drop will erase that working copy. Recover it with `abort_and_take_existing_share()` before dropping the protocol, then retry. Recovering the share marks an in-flight session as failed (it cannot continue without the share) — hence the `abort_` prefix.
