...
impl DilithiumSignProtocol {
    ...
    fn deserialize_message(&self, data: &[u8]) -> Result<SigningMessage, SignProtocolError> {
        ...
        match tag {
            1 => {
                ...
                Ok(SigningMessage::Round1(Round1Broadcast {
                    party_id,
                    commitment_hash,
                }))
            }
            2 => {
                ...
                Ok(SigningMessage::Round2(Round2Broadcast {
                    party_id,
                    commitment_data,
                }))
            }
            3 => {
                ...
                Ok(SigningMessage::Round3(Round3Broadcast {
                    party_id,
                    response,
                }))
            }
            _ => Err(SignProtocolError::SerializationError(format!(
                "Unknown message tag: {}",
                tag
            ))),
        }
    ...
    pub fn poke(&mut self) -> Result<Action<Signature>, SignProtocolError> {
        match &self.state {
            ...
            SignProtocolState::Failed(msg) => {
                Err(SignProtocolError::ProtocolFailed(msg.clone()))
            }
        }
    }
...
pub fn run_local_signing(
    ...
    if num_parties < 2 {
        return Err(SignProtocolError::MissingData(
            "Need at least 2 signers".to_string(),
        ));
    }
