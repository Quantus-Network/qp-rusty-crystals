...
crate::frontend::define_ml_dsa!(ml_dsa_65, crate::params::ml_dsa_65);
...
mod tests {
	...
	fn self_verify_roundtrip() {
		...
		let msg = b"ml-dsa-65 smoke test";
		let sig = keys.sign(msg, None, None).unwrap();
		assert!(keys.verify(msg, &sig, None));
		assert_eq!(sig.len(), SIGNBYTES);
		assert_eq!(SIGNBYTES, 3309);
		assert_eq!(super::PUBLICKEYBYTES, 1952);
		assert_eq!(super::SECRETKEYBYTES, 4032);
	}
